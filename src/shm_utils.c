...
void inline open_semaphore(char *const sem_path, sem_t **semaphore) {
...
void inline close_semaphore(sem_t *semaphore, char *const sem_path) {
    ...
    if (sem_unlink(sem_path) == -1) {
        fprintf(stderr, "Error: Could not unlink semaphore during close_semaphore\n");
        perror("sem_unlink");
        exit(EXIT_FAILURE);
    }
}
